/**
 * Date plugin for Vague
 *
 * Provides:
 * 1. Date generators with day-of-week filtering
 * 2. Duration functions for date arithmetic
 *
...
// ============================================
// Duration type for date arithmetic
// ============================================

/**
 * Duration object representing a time offset.
 * When added to a date string, produces a new date string.
 */
...
/**
 * Check if a value is a Duration object
 */
export function isDuration(value: unknown): value is Duration {
  return (
    ...
    value !== null &&
    '__type' in value &&
    (value as Duration).__type === 'Duration'
  );
}
 ...
 */
function days(n: unknown): Duration {
 ...
 */
function weeks(n: unknown): Duration {
  const num = typeof n === 'number' ? n : parseFloat(String(n));
  return {
    __type: 'Duration',
...
/**
 * Create a duration of N months (approximate: 30 days)
 */
function months(n: unknown): Duration {
  const num = typeof n === 'number' ? n : parseFloat(String(n));
  return {
    __type: 'Duration',
    milliseconds: num * 30 * 24 * 60 * 60 * 1000,
  };
}

/**
 * Create a duration of N years (approximate: 365 days)
 */
function years(n: unknown): Duration {
  const num = typeof n === 'number' ? n : parseFloat(String(n));
  return {
    __type: 'Duration',
    milliseconds: num * 365 * 24 * 60 * 60 * 1000,
  };
}
 ...
 */
function hours(n: unknown): Duration {
  const num = typeof n === 'number' ? n : parseFloat(String(n));
  return {
    __type: 'Duration',
 ...
 */
function minutes(n: unknown): Duration {
  const num = typeof n === 'number' ? n : parseFloat(String(n));
  return {
    __type: 'Duration',
...
/**
 * Add a duration to a date string, returning a new date string
 */
export function addDurationToDate(dateStr: string, duration: Duration): string {
  const date = new Date(dateStr);
  const newDate = new Date(date.getTime() + duration.milliseconds);

  // If original was date-only (no time component), return date-only
...
/**
 * Subtract a duration from a date string, returning a new date string
 */
export function subtractDurationFromDate(dateStr: string, duration: Duration): string {
  const date = new Date(dateStr);
  const newDate = new Date(date.getTime() - duration.milliseconds);
...
// ============================================
// Date generation with day-of-week filtering
// ============================================
...
export const datePlugin: VaguePlugin = {
  name: 'date',
  generators: {
    // Date generation
...
/**
 * Shorthand generators that don't require the "date." prefix
 */
export const dateShorthandPlugin: VaguePlugin = {
  name: 'date-shorthand',
  generators: {
    weekday: wrap(generateWeekday),
    weekend: wrap(generateWeekend),
...
export const datesPlugin = datePlugin;
export const datesShorthandPlugin = dateShorthandPlugin;
