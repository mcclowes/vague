...
describe("OpenAPIExamplePopulator", () => {
  ...
  describe("populate - path-level examples", () => {
    it("populates response examples for array responses", () => {
      ...
      const response = document.paths?.["/pets"]?.get?.responses?.["200"] as OpenAPIV3.ResponseObject;
    ...
    it("populates request body examples", () => {
      ...
      const requestBody = document.paths?.["/pets"]?.post?.requestBody as OpenAPIV3.RequestBodyObject;
  ...
  describe("document cloning", () => {
    it("does not mutate the original document", () => {
      ...
      populator.populate(doc, { pets: [{ id: 1, name: "Fluffy" }] }, {
        externalRefs: false,
        exampleCount: 1,
        mapping: { pets: "Pet" },
      });
